...
function Update-NetworkLabels {
    $lblChannel1.Text = "Channel A -> Z : local $($script:currentLocalIP)`:$($script:currentAtoZSourcePort) -> remote $RemoteIP`:$($script:currentAtoZDestPort)"
    $lblChannel2.Text = "Channel Z -> A : remote $RemoteIP`:$($script:currentZtoASourcePort) -> local $($script:currentLocalIP)`:$($script:currentZtoADestPort)"
    $statusLabel.Text = "Send(A->Z): $($script:currentLocalIP)`:$($script:currentAtoZSourcePort) -> $RemoteIP`:$($script:currentAtoZDestPort) | Recv(Z->A): $($script:currentLocalIP)`:$($script:currentZtoADestPort)"
}
...
function Initialize-Network {
    ...
    $script:sendEndpoint = New-Object System.Net.IPEndPoint($script:remoteAddress, $script:currentAtoZDestPort)
    ...
    while (-not $sender.CancellationPending) {
        try {
            if (($null -ne $script:recvUdp) -and ($script:recvUdp.Available -gt 0)) {
                ...
                if (($anyEndpoint.Port -eq $script:currentZtoASourcePort) -and ($anyEndpoint.Address.Equals($script:remoteAddress))) {
