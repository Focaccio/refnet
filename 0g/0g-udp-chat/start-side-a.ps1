...
$lblChannel1.Text = "Channel A -> Z : local $LocalIP`:$AtoZSourcePort -> remote $RemoteIP`:$AtoZDestPort"
...
$lblChannel2.Text = "Channel Z -> A : remote $RemoteIP`:$ZtoASourcePort -> local $LocalIP`:$ZtoADestPort"
...
$sendEndpoint = New-Object System.Net.IPEndPoint($remoteAddress, $AtoZDestPort)
...
$sendUdp.Client.Bind((New-Object System.Net.IPEndPoint($localAddress, $AtoZSourcePort)))
...
$recvUdp.Client.Bind((New-Object System.Net.IPEndPoint($localAddress, $ZtoADestPort)))

$statusLabel.Text = "Send(A->Z): $LocalIP`:$AtoZSourcePort -> $RemoteIP`:$AtoZDestPort | Recv(Z->A): $LocalIP`:$ZtoADestPort"
    ...
    while (-not $sender.CancellationPending) {
        try {
            if ($recvUdp.Available -gt 0) {
                ...
                if (($anyEndpoint.Port -eq $ZtoASourcePort) -and ($anyEndpoint.Address.Equals($remoteAddress))) {
