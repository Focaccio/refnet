    ...
    while (-not $sender.CancellationPending) {
        try {
            if (($null -ne $script:recvUdp) -and ($script:recvUdp.Available -gt 0)) {
                ...
                $isExpectedPeer = Test-ExpectedPeer -Endpoint $anyEndpoint -ExpectedPort $script:currentZToASourcePort -ExpectedIP $script:currentSideZIP
                $form.BeginInvoke([Action]{
                    if ($msg -eq $remoteProbePayloadText) {
                        if ($isExpectedPeer) {
                            Add-ChatLine -Prefix "[Probe RX $($anyEndpoint.ToString()) Z->A]" -Message $msg
                        } else {
                            Add-ChatLine -Prefix "[Probe RX Unverified $($anyEndpoint.ToString()) Z->A]" -Message $msg
                        }
                    } else {
                        if ($isExpectedPeer) {
                            Add-ChatLine -Prefix "[$($anyEndpoint.ToString()) Z->A]" -Message $msg
                        } else {
                            Add-ChatLine -Prefix "[Unverified $($anyEndpoint.ToString()) Z->A]" -Message $msg
                        }
                    }
                }) | Out-Null
            } else {
